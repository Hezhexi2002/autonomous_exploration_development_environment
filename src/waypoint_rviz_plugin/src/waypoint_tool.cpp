...
void WaypointTool::updateTopic()
{
  ...
  sub_ = raw_node->template create_subscription<nav_msgs::msg::Odometry>(robot_id + "/state_estimation", 5 ,std::bind(&WaypointTool::odomHandler,this,std::placeholders::_1));
  
  pub_ = raw_node->template create_publisher<geometry_msgs::msg::PointStamped>(robot_id + "/way_point", qos_profile_);
